...
import { ref } from 'vue'
import { getTemplateDetail, createProjectFromTemplate } from '../../api'
const id = ref('')
...
onLoad(async (q) => {
  ...
  detail.value = await getTemplateDetail(id.value)
  tpl.value = detail.value
})

async function useThis(){
  const { projectId } = await createProjectFromTemplate(id.value)
  uni.navigateTo({ url:`/src/pages/editor/index?pid=${projectId}` })
}
